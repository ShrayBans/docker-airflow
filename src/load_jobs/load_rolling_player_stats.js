...
async function run() {
	...
	let pstDate = moment(new Date()).toDate()
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			const interval = setInterval(async () => {
				pstDate = moment.tz(moment(pstDate).subtract(1, 'day').toDate(), "America/Los_Angeles").format("YYYYMMDD");
				console.log(`Checking if redis stats have been found for date of: ${pstDate}`);

				// Check if data has been loaded
				const homeTeamPlayers = await hmgetRedisClient(redisClient, `teamId:1610612748:day:${pstDate}:top10`, "reb");
				if (_.size(homeTeamPlayers) > 0) {
					console.log(`Redis stats have been found for: ${pstDate}... exiting`);
					return resolve(true)
				}
				console.log(`Redis stats have not been found for: ${pstDate}`);
